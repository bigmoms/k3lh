...
class IadlController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('lingkungan.iadl.index', compact('menus'));
    }
    ...
    public function fetch(Request $request)
    {
        ...
        $query = DB::table('aktivitas_iadl as a')
            ->join('users as u', 'a.user_id', '=', 'u.id')
            ->join('identifikasi_aspek_dampak_lingkungan as k', 'a.id', '=', 'k.aktivitas_id');
        ...
        <div class="dropdown text-center">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                <li>
                    <a class="dropdown-item d-flex align-items-center" href="' . route('lingkungan.iadl.show', $encodedId) . '">
                        <i class="fas fa-eye me-2 text-primary"></i> Detail
                    </a>
                </li>
                <li>
                    <a class="dropdown-item d-flex align-items-center" href="' . route('lingkungan.iadl.duplicate', $encodedId) . '">
                        <i class="fas fa-copy me-2 text-secondary"></i> Salin
                    </a>
                </li>
                <li>
                    <a class="dropdown-item d-flex align-items-center" href="' . route('lingkungan.iadl.download', $encodedId) . '" target="_blank">
    ...
    public function create(Request $request)
    {
        ...
        return view('lingkungan.iadl.create', compact(
            'menus',
            'lokasi',
            'divisi',
        ));
    ...
    //     return view('lingkungan.iadl.row', compact('index'));
    // }

    public function aktifitasRow(Request $request)
    {
        $index = $request->get('index');
        return view('lingkungan.iadl.aktifitas', ['index' => $index]);
    }
    ...
    public function kegiatanRow(Request $request)
    {
        ...
        return view('lingkungan.iadl.kegiatan', compact('aidx', 'kidx'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $jumlahInputanDivisi = DB::table('aktivitas_iadl')
        ...
        $existingBatch = DB::table('aktivitas_iadl')
        ...
        $urutanDivisi = DB::table('aktivitas_iadl')
            ->where('divisi_id', $divisiId)
            ->where('tahun', $tahun)
            ->pluck('urutan_input')
            ->toArray();
        ...
        try {
            foreach ($request->data as $item) {
                $aktivitasId = DB::table('aktivitas_iadl')->insertGetId([
                    'user_id' => $userId,
                    'batch_id' => $batchId,
                ...
                $this->insertIADLKegiatan($aktivitasId, $item['kegiatan'] ?? []);
            }
    ...
    private function insertIADLKegiatan(int $aktivitasId, array $kegiatanList)
    {
        foreach ($kegiatanList as $row) {
            DB::table('identifikasi_aspek_dampak_lingkungan')->insert([
                'aktivitas_id' => $aktivitasId,
                'aspek_lingkungan' => $row['aspek_lingkungan'] ?? null,
                'dampak_lingkungan' => $row['dampak_lingkungan'] ?? null,
                'risiko_lingkungan' => $row['risiko_lingkungan'] ?? null,
                'na_be' => $row['na_be'] ?? null,
                'no_dampak' => $row['no_dampak'] ?? null,
                'l_awal' => $row['l_awal'] ?? null,
                'c_awal' => $row['c_awal'] ?? null,
                'total_awal' => $row['total_awal'] ?? null,
                'tingkat_risiko_awal' => $row['tingkat_risiko_awal'] ?? null,
                'pengendalian_saat_ini' => $row['pengendalian_saat_ini'] ?? null,
                'l_akhir' => $row['l_akhir'] ?? null,
                'c_akhir' => $row['c_akhir'] ?? null,
                'total_akhir' => $row['total_akhir'] ?? null,
                'tingkat_risiko_akhir' => $row['tingkat_risiko_akhir'] ?? null,
    ...
    public function show($id)
    {
        ...
        $item = IdentifikasiAspekDampakLingkungan::with('aktivitas.lokasiPengukuran')->findOrFail($decodedId);
        ...
        if ($isInternal && $lokasi && $tahun && $urutanInput) {
            $data = IdentifikasiAspekDampakLingkungan::with(['aktivitas.lokasiPengukuran', 'aktivitas.user'])
        ...
        $dataHighRisk = $data->filter(fn($item) => $item->tingkat_risiko_akhir === 'H');
        $menus = request()->get('dtmenus');
        $encodedId = $id;
        return view('lingkungan.iadl.show', compact(
            'groupedData',
            'dataHighRisk',
            'lokasi',
            'tahun',
    ...
    public function updatePeraturan(Request $request, $id)
    {
        ...
        foreach ($validated['data'] as $itemId => $fields) {
            ...
            if (!empty($updateData)) {
                IdentifikasiAspekDampakLingkungan::where('id', $itemId)->update($updateData);
            }
        }
    ...
    public function duplicate($id)
    {
        ...
        $aktivitasId = DB::table('identifikasi_aspek_dampak_lingkungan')
            ->where('id', $decodedId)
            ->value('aktivitas_id');

        if (!$aktivitasId) abort(404, 'Data tidak ditemukan.');

        $aktivitasPertama = DB::table('aktivitas_iadl')
        ...
        $aktivitasList = DB::table('aktivitas_iadl')->where('batch_id', $batchId)->get();
        $structured = [];

        $tahunSekarang = now()->year;

        foreach ($aktivitasList as $aktivitas) {
            $kegiatan = DB::table('identifikasi_aspek_dampak_lingkungan')
        ...
        return view('lingkungan.iadl.create', [
            'menus' => request()->get('dtmenus'),
    ...
    public function download($id)
    {
        ...
        $pdf = Pdf::loadView('lingkungan.iadl.pdf', [
            'groupedData' => $groupedData,
            'lokasi' => $lokasi,
            'tahun' => $tahun,
        ])->setPaper('A4', 'landscape');

        return $pdf->stream('Detail_IADL.pdf');
    }
}
