...
@section('scripts')
    ...
    <script>
        ...
        function getJson() {
            $('#mrx').DataTable({
                responsive: true,
                ajax: "{{ url('admin/setting_notifikasi/json') }}",
                columns: [{
                        data: 'id',
        ...
        function modal(id) {
            $("#modal-x").modal('show');
            $("#view-modal").load("{{ url('admin/setting_notifikasi/modal') }}?id=" + id);
        }
        ...
        $('#btn-submit').on('click', () => {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ url('admin/setting_notifikasi/store') }}",
        ...
        function hapus(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'GET',
                        url: "{{ url('admin/setting_notifikasi/hapus') }}",
                        data: {
                            id: id
                        },
                        success: function(res) {
        ...
        function push(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'GET',
                        url: "{{ url('admin/setting_notifikasi/push') }}",
                        data: {
                            id: id
                        },
                        success: function(res) {
                        ...
                        error: function(xhr) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Gagal!',
                                html: `<strong>${message}</strong><br>${errorDetail}`,
                            });
                        }
                    });
                }
